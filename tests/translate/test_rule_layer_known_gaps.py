...
def test_IndexedPredicateGetReceivers_fixture_translates_without_unhandled_diagnostics() -> None:
    parsed = parse_file(
        CFG_FIXTURES / "corpus" / "constructs" / "IndexedPredicateGetReceivers.java",
    )
    result = translate_skeleton_with_diagnostics(parsed, extract_symbols(parsed), CFG)
    ...
    assert "return flags.get(index)" in result.source
    assert "bit_set[index]" not in result.source
    assert "ambiguous get invocation requires receiver collection type" not in result.source
    assert "__j2py_todo__" not in result.source


def test_ReceiverGetCategories_fixture_translates_without_unhandled_diagnostics() -> None:
    parsed = parse_file(CFG_FIXTURES / "corpus" / "constructs" / "ReceiverGetCategories.java")
    result = translate_skeleton_with_diagnostics(parsed, extract_symbols(parsed), CFG)
...
def test_ClassKeyedRegistryGet_fixture_translates_without_unhandled_diagnostics() -> None:
    parsed = parse_file(CFG_FIXTURES / "corpus" / "constructs" / "ClassKeyedRegistryGet.java")
    result = translate_skeleton_with_diagnostics(parsed, extract_symbols(parsed), CFG)
