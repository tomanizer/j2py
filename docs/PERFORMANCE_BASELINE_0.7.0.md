# 0.7.0 performance baseline

This page records the 0.7.0 release performance baseline for representative local
translation and reporting paths. The numbers are approximate wall-clock measurements from
one maintainer machine, intended to expose obvious bottlenecks and give release reviewers
a concrete reference point.

## Environment

- Date: 2026-06-19
- Baseline commit: `5261146`
- OS: macOS Darwin 22.6.0 x86_64
- Python: 3.11.14 from the checked-in `.venv`
- Output root: `/private/tmp/j2py-issue-585-perf`
- LLM calls: disabled
...
as the largest quality backlog clusters; those are translation-quality priorities rather
...
caches, or a syntax-only validation mode, would change validation semantics or operational
complexity and should be handled through a dedicated design issue if the release profile
shows real user pain.
