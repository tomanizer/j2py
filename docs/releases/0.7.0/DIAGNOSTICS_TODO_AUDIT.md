# 0.7.0 diagnostics and TODO wording audit

This audit records the release-facing diagnostics and TODO messages users are likely to
see in 0.7.0 boundary cases. The goal is honest output: generated Python should stay
...
- JDBC placeholder output now points to `import_map` / `type_map` or a SQLAlchemy/project
  DB shim.
- Unsupported Spring JDBC calls now emit TODO text that matches the diagnostic reason
  instead of always showing the RowMapper callback message.
- RowMapper TODO wording now names the concrete manual action: port to SQLAlchemy row
  mapping or a project DB facade.
- Spring JDBC bean diagnostics now tell users to wire a SQLAlchemy `Engine`,
  `Connection`, or `Session` dependency in project code.
- Spring Data JPQL and derived-query TODOs now say to manually port the repository query
