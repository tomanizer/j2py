# 0.8.1 release candidate checklist

This checklist records the package build and clean-environment install verification for
the 0.8.1 release-candidate path. The verification uses the final `0.8.1` package
...
- Normal pytest passed: `3285 passed, 8 skipped, 106 deselected, 1 xfailed`.
- Future target xfail suite passed with the expected single xfail.
- Behavior corpus passed: `91 passed, 1 deselected`.
- Spring PetClinic smoke passed: `3 passed`.
- Version consistency passed for the release package version: `0.8.1`.
- Import and CLI smoke passed.
- Built artifacts:
  - `dist/j2py_converter-0.8.1-py3-none-any.whl`
  - `dist/j2py_converter-0.8.1.tar.gz`
- Sdist hygiene passed: `dist/j2py_converter-0.8.1.tar.gz: clean`.
...
```bash
tar -tf dist/j2py_converter-0.8.1.tar.gz |
...
```bash
python3 -m venv /private/tmp/j2py-0.8.1-core-venv-release71
/private/tmp/j2py-0.8.1-core-venv-release71/bin/python -m pip install \
  dist/j2py_converter-0.8.1-py3-none-any.whl
```
...
```text
/private/tmp/j2py-0.8.1-core-venv-release71/lib/python3.14/site-packages/j2py/__init__.py
```
...
- A self-contained `HelloWorld.java` directory translated with `--no-llm --no-validate`
  and produced `/private/tmp/j2py-0.8.1-core-smoke-release71/out/com/example/HelloWorld.py`.
...
```bash
python3 -m venv /private/tmp/j2py-0.8.1-spring-venv-release71
/private/tmp/j2py-0.8.1-spring-venv-release71/bin/python -m pip install \
  "dist/j2py_converter-0.8.1-py3-none-any.whl[spring]"
```

Installed package location:

```text
/private/tmp/j2py-0.8.1-spring-venv-release71/lib/python3.14/site-packages/j2py/__init__.py
```
...
No package-install blocker was found in this verification. Create the GitHub Release tag
only after the final release PR is green and merged.
