# 0.8.1 release test coverage inventory

This inventory maps release-facing 0.8.1 claims to the checked-in evidence that backs
them. It is a maintenance aid for the release notes, README, case studies, wiring docs,
and Spring conversion guide; it does not expand product scope.
...
| Nested class literals inside file-declared nested types are enclosing-qualified instead of emitted as undefined bare names. | `CHANGELOG.md`, `docs/releases/0.8.1/RELEASE_NOTES.md` | `tests/translate/skeleton/test_expressions.py::test_nested_type_class_literals_are_enclosing_qualified`; `make release-check` |
| `EnumSet` factory lowering preserves Java-like order and string form, and `Math.incrementExact` records the overflow semantic warning instead of diverging silently. | `CHANGELOG.md`, `docs/releases/0.8.1/RELEASE_NOTES.md` | `tests/translate/test_runtime_dispatch.py`; `tests/fixtures/behavior/enum_set_iteration_order/Main.java`; `make test-behavior`; `make release-check` |
| WordUtils delimiter and code-point lowering covers Predicate call sites, collection `::contains`, `Character.toLowerCase(codePoint)`, and no-arg null-forwarding varargs overloads. | `docs/CASE_STUDY_COMMONS_TEXT_CASEUTILS.md`, `CHANGELOG.md` | `tests/case_study/test_commons_text_wordutils_case_study.py`; `tests/translate/skeleton/test_expressions.py`; `tests/translate/skeleton/test_overloads.py`; `make release-check` |
| The java-semver scoped external case study covers the `Version` / parser core loop with zero residual translator-defect patches and zero generated-output `F821` findings for that bundle. | `docs/CASE_STUDY_JSEMVER.md`, `CHANGELOG.md` | `tests/case_study/test_jsemver_case_study.py`; `tests/case_study/jsemver_harness.py`; `tests/fixtures/case_study/jsemver/`; `make release-check` |
...
| Raw JDBC, native driver bridges, framework container behavior, and production SQLAlchemy engine or session policy remain project-owned boundaries. | `docs/POSITIONING.md`, `docs/examples/SPRING_MAPPING_COOKBOOK.md`, `docs/releases/0.8.1/RELEASE_NOTES.md` | `tests/translate/test_platform_imports.py::test_raw_jdbc_fixture_preserves_boundaries_without_java_imports`; `tests/fixtures/java/RawJdbcBoundary.java`; `tests/translate/skeleton/test_spring_wiring_plugin.py::test_jdbc_bean_methods_emit_topology_metadata_and_boundary_warning` |
| External case studies provide bounded runtime evidence for java-semver, Commons Text, and Apache Commons Codec `Hex` without claiming full-library automatic migration. | `docs/CASE_STUDY_JSEMVER.md`, `docs/CASE_STUDY_COMMONS_TEXT_CASEUTILS.md`, `docs/CASE_STUDY_COMMONS_CODEC_HEX.md` | `tests/case_study/test_jsemver_case_study.py`; `tests/case_study/test_commons_text_caseutils_case_study.py`; `tests/case_study/test_commons_text_wordutils_case_study.py`; `tests/case_study/test_commons_codec_hex_case_study.py`; `tests/fixtures/case_study/` |
| Package metadata supports core installs, optional Spring trial installs, and optional LLM-provider extras without pulling Spring runtime dependencies into the default import path. | `docs/INSTALL.md`, `docs/RELEASING.md`, `docs/releases/0.8.1/RELEASE_NOTES.md` | `tests/packaging/test_pyproject_dependencies.py`; `make release-check`; `dist-check`; publish workflow `release-test` and `release-dist` jobs |
| Release validation exercises the normal test suite, graduated target contracts, behavior corpus, Spring smoke, version checks, import smoke, package build, sdist hygiene, and twine metadata checks. | `docs/RELEASING.md`, `docs/releases/0.8.1/RELEASE_NOTES.md` | `make release-check`; `Makefile`; `.github/workflows/publish.yml`; `tests/test_ci_workflows.py`; `tests/packaging/test_check_sdist_hygiene.py`; `tests/packaging/test_check_release_versions.py` |
...
When editing 0.8.1 release-facing docs:
