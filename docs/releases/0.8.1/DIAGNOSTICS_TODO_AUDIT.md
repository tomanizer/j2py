# 0.8.1 diagnostics and TODO wording audit

This audit records the release-facing diagnostics and TODO messages users are likely to
see in 0.8.1 boundary cases. The goal is honest output: generated Python should stay
...
| `Math.incrementExact` overflow | `Math.incrementExact overflow semantics are not preserved; Python ints do not overflow` | semantic warning | Manually enforce Java overflow behavior if the translated code depends on `ArithmeticException`. |
...
- `Math.incrementExact` lowering now emits a semantic warning for the Java overflow
  contract instead of silently presenting Python's unbounded integer behavior as fully
  equivalent.
