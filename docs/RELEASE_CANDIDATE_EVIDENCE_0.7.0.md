...
the 0.7.0 release-candidate path. The verification used the final `0.7.0` package
...
- Normal pytest passed: `2977 passed, 8 skipped, 105 deselected, 1 xfailed`.
...
- Version consistency passed for the release package version: `0.7.0`.
- Import and CLI smoke passed.
- Built artifacts:
  - `dist/j2py_converter-0.7.0-py3-none-any.whl`
  - `dist/j2py_converter-0.7.0.tar.gz`
- Sdist hygiene passed: `dist/j2py_converter-0.7.0.tar.gz: clean`.
...
```bash
tar -tf dist/j2py_converter-0.7.0.tar.gz |
...
```bash
python3 -m venv /private/tmp/j2py-0.7.0-core-venv
/private/tmp/j2py-0.7.0-core-venv/bin/python -m pip install \
  dist/j2py_converter-0.7.0-py3-none-any.whl
```
...
```text
/private/tmp/j2py-0.7.0-core-venv/lib/python3.14/site-packages/j2py/__init__.py
```
...
- A self-contained `HelloWorld.java` directory translated with `--no-llm --no-validate`
  and produced `/private/tmp/j2py-0.7.0-core-smoke/out/com/example/HelloWorld.py`.
...
```bash
python3 -m venv /private/tmp/j2py-0.7.0-spring-venv
/private/tmp/j2py-0.7.0-spring-venv/bin/python -m pip install \
  "dist/j2py_converter-0.7.0-py3-none-any.whl[spring]"
```
