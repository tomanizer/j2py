...
def _translate_anonymous_class(
    ...
    for member in body_node.named_children:
        ...
        if member.type == "method_declaration":
            if wrote_member:
                helper_lines.append("")
            helper_lines.extend(_anonymous_method_lines(member, ctx))
            wrote_member = True
            continue
        ctx.diagnostics.record(
            member,
...
def _anonymous_method_lines(method: JavaNode, ctx: TranslationContext) -> list[str]:
